...
function vector ComputeTrajectoryByTime( vector StartPosition, vector EndPosition, float fTimeEnd  )
{
	...
	if ( PhysicsVolume.IsA( 'KFPhysicsVolume' ) && StartPosition.Z < EndPosition.Z 
		 && PhysicsVolume.Gravity.Z < class'PhysicsVolume'.default.Gravity.Z
		 && Mass > 900 ) // Just checking mass to be extra-cautious.
		NewVelocity.Z += 90;
...
function bool OnlyEnemyAround( Pawn Other )
{
	...
	for( C=Level.ControllerList; C!=None; C=C.NextController )  {
		if ( C.bIsPlayer && C.Pawn != None && C.Pawn != Other 
			 && ((VSize(C.Pawn.Location-Location) < 1500 && FastTrace(C.Pawn.Location, Location))
...
{
	if( AnimName=='MeleeImpale' || AnimName=='MeleeClaw' 
		|| AnimName=='transition' /*|| AnimName=='FireMG'*/  )  {
		AnimBlendParams(1, 1.0, 0.0,, SpineBone1);
...
{
    if ( TestAnim == 'FireMG' || TestAnim == 'PreFireMG' || TestAnim == 'PreFireMissile' 
		 || TestAnim == 'FireEndMG' || TestAnim == 'FireEndMissile' || TestAnim == 'Heal' 
		 || TestAnim == 'KnockDown' || TestAnim == 'Entrance' || TestAnim == 'VictoryLaugh' 
		 || TestAnim == 'RadialAttack' 
		/*TestAnim == 'MeleeImpale' || TestAnim =='MeleeClaw' || TestAnim =='transition' ||*/ )
        Return True;
	else
		Return False;
}
...
function bool ShouldChargeFromDamage()
{
    ...
    if( (SyringeCount == 0 && Health < HealingLevels[0]) 
        ...
        Return False;
			 ...
			 && (Level.TimeSeconds - LastForceChargeTime) > (5.0 + 5.0 * FRand()) )
        Return True;
	else
		Return False;
}
...
event TakeDamage( int Damage, Pawn InstigatedBy, Vector Hitlocation, Vector Momentum, class<DamageType> damageType, optional int HitIndex)
{
	...
	if ( Health <= 0 || SyringeCount == 3 || IsInState('Escaping') 
		 || IsInState('KnockDown') || IsInState('RadialAttack') 
